...
####################################################
###### GLOBAL SENSITIVITY ANALYSIS SIMULATIONS ######
####################################################
...
sensitivity_samples <- readRDS("other/sensitivity_analysis_global_samples.rds") %>%
  mutate(output_suffix = spinup_suffix)

# create df of all seeds x parameters for sensitivity analysis
all_normal_run <- merge(all_normal, sensitivity_samples, by = NULL)
...
process_in_batches(all_normal_run[sample(nrow(all_normal_run)), ], batch_size = 1000, slim_function = run_slim_normal, num_processes = num_processes)
