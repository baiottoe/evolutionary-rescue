...
generateLandscape <- function(seed, slope, curvature, amplitude, aclength, minSize=64, periodic_x=F)
{
  ...
  if (periodic_x == T && (slope != 0.0 | curvature != 0.0))
	...
	else
	{
		n <- ncol(noise)
		x <- xvec(n)
		mat <- matrix(slope*x + curvature*x^2, n, n, byrow=TRUE) + noise
		
		mat_as_v <- scale(c(mat))
		
		num_rows <- num_cols <- sqrt(length(mat_as_v))
		
		# Convert the vector back to a square matrix
		mat_scaled <- (matrix(mat_as_v, nrow = num_rows, ncol = num_cols))
		
		return(mat_scaled)
	}
}

generateHL <- function(seed, slope, curvature, amplitude, aclength, minSize=64, periodic_x=F, proportion_destroyed=0.5)
{
  hl <- generateLandscape(seed, slope, curvature, amplitude, aclength, minSize=minSize, periodic_x=periodic_x)
  
  # reclassify based on amount of habitat lost
  quantileCut <- quantile(hl, proportion_destroyed)
  
  # Replace values below 66th percentile with 0 and above with 1 (0 means HL)
  # must check if quantile cut is >1, because if so, need to perform setting operation to 0 first (otherwise, all values will be set to 0 at end)
  if (quantileCut > 1) {
    hl[hl < quantileCut] <- 0 
    hl[hl >= quantileCut] <- 1
  } else {
