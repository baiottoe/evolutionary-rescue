...
file_list <- list.files(path = "outputs/spatial/simulation_output", pattern = "\\CC3.txt$", full.names = TRUE)
...
# Check for existing outputs and filter
file_list <- list.files(path = "outputs/spatial/simulation_output", pattern = "\\CC0.txt$", full.names = TRUE)
if (length(file_list) > 0) {
  data_list <- lapply(file_list, read.csv)
  all_out <- do.call(rbind, data_list)
  all_out <- all_out %>%
